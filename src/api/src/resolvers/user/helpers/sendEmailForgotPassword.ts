...
export const sendEmailForgotPassword: SendEmailForgotPassword = async (email, captchaSolution, token) => {
  ...
  const message: MailDataRequired = {
    ...
    text: `Please enter the following: ${captchaSolution} on https://near.academy/reset-password?key=${token}`,
    html: `Please enter the following: ${captchaSolution} <br />on <a href="https://near.academy/reset-password?key=${token}">https://near.academy/reset-password?key=${token}</a>`,
  }
